...
excerpt: Tree-sitter allows cecli to build a repo map that better summarizes large code bases.
...
Cecli now uses it to help GPT better comprehend, navigate
and edit code in larger repos.

*To code with GPT-4 using the techniques discussed here, just install [cecli](https://cecli.dev/docs/install.html).*
...
Cecli also strives to reduce the manual work involved in
coding with AI.
So in an ideal world, we'd like cecli to automatically
identify and provide the needed code context.

## Using a repo map to provide context

Cecli sends a **repo map** to GPT along with
...
Mapping out the repo like this provides some key benefits:
  ...
  - If it needs to see more code, GPT can use the map to figure out by itself which files it needs to look at in more detail. GPT will then ask to see these specific files, and cecli will automatically add them to the chat context.
...
Cecli uses all of these definitions and references to
...
[ctags based map](https://cecli.dev/docs/ctags.html)
that cecli originally used.
...
- Thanks to `py-tree-sitter-languages`, we get full support for many programming languages via a python package that's automatically installed as part of the normal `python -m pip install -U bright-vision-core`.
...
- Tree-sitter integration is a key enabler for future work and capabilities for cecli.
...
Right now, cecli relies on the user to specify which source files
will need to be modified to complete their request.
Users manually "add files to the chat" using cecli's `/add` command,
...
just install [cecli](https://cecli.dev/docs/install.html).

## Credits

Cecli uses
[modified versions of the tags.scm files](https://github.com/dwash96/cecli/tree/main/cecli/queries)
from these
