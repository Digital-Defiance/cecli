...
excerpt: Cecli now lints code after every LLM edit and automatically fixes errors, using tree-sitter and AST-aware code context.
...
[![Linting code](/assets/linting.jpg)](https://cecli.dev/assets/linting.jpg)

# Linting code for LLMs with tree-sitter

Cecli now lints your code after every LLM edit, and offers to automatically fix
any linting errors.
You can also use cecli's lint-and-fix functionality on your source files any time
...
Cecli ships with basic linters built with tree-sitter that support
...
You can also configure cecli to use your preferred linters.
This allows cecli to check for a larger class of problems, keep the code style
aligned with the rest of your team, etc.

## Linting and fixing your code

Cecli now lints each source file after it applies the edits
suggested by an LLM.
If problems are found, cecli will ask if you'd like it to
attempt to fix the errors.
If so, cecli will send the LLM a report of the lint errors
and request changes to fix them. This process may iterate a few times
as the LLM works to fully resolve all the issues.

You can also lint and fix files any time, on demand from within the cecli chat or via the
command line:
...
- From the command line, you can run `cecli --lint` to lint and fix
...
linting errors.
LLM's don't have the luxury of using an IDE, so cecli sends
...
Cecli augments the raw linter by
displaying and
highlighting the lines that have errors within their
containing functions, methods, classes.
To do this, cecli uses tree-sitter to obtain the code's AST and analyzes it
...
Cecli comes batteries-included with built in linters for
[most popular programming languages](https://cecli.dev/docs/languages.html).
This provides wide support for linting without requiring
users to manually install a linter and configure it to work with cecli.

Cecli's built in language-agnostic linter uses tree-sitter to parse
...
Cecli simply uses these `ERROR` nodes to identify all the lines
