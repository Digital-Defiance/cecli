...
Cecli now has experimental support for using two models to complete each coding task:
...
[cecli's code editing benchmark](/docs/benchmarks.html#the-benchmark).
...
This approach has recently become attractive for cecli due to 
...
incompatible with cecli's goal of providing an interactive,
pair programming AI coding experience.

## Code reasoning and code editing

Normally cecli asks the model to solve a coding problem in one prompt,
asking the LLM to explain the solution and return 
a well formatted series of file edits.
All of [cecli's editing formats](/docs/more/edit-formats.html)
require the LLM to return source code edits in a specific text
format, so that cecli can process the edits and apply them to the local source files.
...
The development version of cecli 
has built in defaults to support Architect/Editor coding with
o1-preview, o1-mini, GPT-4o and Claude 3.5 Sonnet.
Run cecli with `--architect` or get started quickly like this:

```
pip install -U bright-vision-core
...
cecli --sonnet --architect

# Work with OpenAI models, using gpt-4o as the Editor
export OPENAI_API_KEY=your-key-goes-here
cecli --4o --architect
cecli --o1-mini --architect
cecli --o1-preview --architect
```

## More info

Cecli has a number of "chat modes", and "architect" is available as a new chat mode.
The `--architect` switch is a shortcut for `--chat-mode architect`.
For more details, see documentation on 
[cecli's chat modes](/docs/usage/modes.html).
...
by itself in cecli's normal "code" editing mode
