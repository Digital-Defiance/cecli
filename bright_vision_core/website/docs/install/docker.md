...
Cecli is available as 2 docker images:

- `paulgauthier/cecli` installs the cecli core, a smaller image that's good to get started quickly.
- `paulgauthier/cecli-full` installs cecli will all the optional extras.
...
launch the cecli core container.

### Cecli core 

```
docker pull paulgauthier/cecli
docker run -it --user $(id -u):$(id -g) --volume $(pwd):/app paulgauthier/cecli --openai-api-key $OPENAI_API_KEY [...other cecli args...]
```

### Full version

```
docker pull paulgauthier/cecli-full
docker run -it --user $(id -u):$(id -g) --volume $(pwd):/app paulgauthier/cecli-full --openai-api-key $OPENAI_API_KEY [...other cecli args...]
```
