...
When the workspace root is a **git superproject** with nested submodules, Vision Core discovers submodule repos, tracks files inside them, and can commit submodule pointer updates from the root. Use a single workspace path with `bright-vision-core-serve --workspace`.
