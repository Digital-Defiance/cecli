...
can also
[download from GitHub](https://github.com/dwash96/cecli/blob/main/bright_vision_core/website/assets/sample.env).

<!--[[[cog
from bright_vision_core.args import get_sample_dotenv
from pathlib import Path
text=get_sample_dotenv()
Path("bright_vision_core/website/assets/sample.env").write_text(text)
cog.outl("```")
cog.out(text)
cog.outl("```")
]]]-->
...
#AIDER_MODEL_SETTINGS_FILE=.cecli.model.settings.yml

## Specify a file with context window and costs for unknown models
#AIDER_MODEL_METADATA_FILE=.cecli.model.metadata.json
...
#AIDER_INPUT_HISTORY_FILE=.cecli.input.history

## Specify the chat history file (default: .cecli.dev.history.md)
#AIDER_CHAT_HISTORY_FILE=.cecli.dev.history.md
