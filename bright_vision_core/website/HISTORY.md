...
description: Release notes and stats on cecli writing its own code.
---
...
Cecli writes most of its own code, usually about 70-80% of the new code in each release.
These
[statistics are based on the git commit history](/docs/faq.html#how-are-the-cecli-wrote-xx-of-code-stats-computed)
of the cecli repo.
...
- Added `/ok` as a shortcut for asking cecli to go ahead with the proposed changes, with optional extra instructions.
- When auto-commits are disabled, cecli can add files outside the git repo and promote read-only files to editable, by Chris McCormick.
- Fixed crashes caused by circular symlink loops when resolving paths, by coder_3.
- Cecli wrote 62% of the code in this release.
...
- Cecli wrote 88% of the code in this release.
...
- Added `--add-gitignore-files` flag to enable adding files listed in .gitignore to Cecli's editing scope, by omarcinkonis.
...
- Cecli wrote 21% of the code in this release.
...
- Cecli wrote 79% of the code in this release.
...
- Prevented Cecli from instructing the LLM to reply in 'C' or 'POSIX' when these are detected as the system locale.
...
- The `cecli scrape` command-line tool will now use Playwright for web scraping if it is available, by Jon Keys.
...
- The `cecli-args` utility (or `python -m cecli.args`) now defaults to printing a sample YAML configuration if no arguments are provided.
...
- Cecli wrote 55% of the code in this release.
...
- Cecli wrote 92% of the code in this release.

### Cecli v0.81.3

- Commit messages generated by cecli are no longer forced to be entirely lowercase, by Peter Hadlaw.
...
- Fix quoting of values containing '#' in the sample `cecli.conf.yml`.
- Add support for Fireworks AI model 'deepseek-v3-0324', by Felix Lisczyk.
- Commit messages generated by cecli are now lowercase, by Anton Ödman.
...
- Cecli wrote 86% of the code in this release.
...
- Handle `FileNotFoundError` if the current working directory is deleted while cecli is running.
- Fix completion menu current item color styling, by Andrey Ivanov.
- Cecli wrote 87% of the code in this release.
...
- Cecli wrote 93% of the code in this release.

### Cecli v0.79.1

- Improved model listing to include all models in fuzzy matching, including those provided by cecli (not litellm).
...
- Cecli wrote 65% of the code in this release.
...
- Big upgrade in [programming languages supported](https://cecli.dev/docs/languages.html) by adopting [tree-sitter-language-pack](https://github.com/Goldziher/tree-sitter-language-pack/).
...
- Cecli wrote 72% of the code in this release.
...
- Cecli wrote 75% of the code in this release.
...
- Cecli wrote 0% of the code in this release.
...
- Cecli will notify you when it's completed the last request and needs your input:
  - Added [notifications when LLM responses are ready](https://cecli.dev/docs/usage/notifications.html) with `--notifications` flag.
...
- Cecli wrote 85% of the code in this release.
...
- Cecli wrote 46% of the code in this release.
...
- Docker containers now set `HOME=/app` which is the normal project mount-point, to persist `~/.cecli`.
...
- Cecli wrote 77% of the code in this release.

### Cecli v0.73.0

- Full support for o3-mini: `cecli --model o3-mini`
...
- Cecli wrote 69% of the code in this release.
...
- Cecli wrote 52% of the code in this release.
...
- Cecli wrote 60% of the code in this release.
...
- Cecli wrote 74% of the code in this release.
...
- [Watch files](https://cecli.dev/docs/usage/watch.html) improvements:
  - Use `# ... AI?` comments to trigger cecli and ask questions about your code.
  - Now watches *all* files, not just certain source files.
  - Use `# AI comments`, `// AI comments`, or `-- AI comments` to give cecli instructions in any text file.
- Full support for Gemini Flash 2.0 Exp:
  - `cecli --model flash` or `cecli --model gemini/gemini-2.0-flash-exp`
- [New `--multiline` flag and `/multiline-mode` command](https://cecli.dev/docs/usage/commands.html#entering-multi-line-chat-messages) makes ENTER a soft newline and META-ENTER send the message, by @miradnanali.
- `/copy-context <instructions>` now takes optional "instructions" when [copying code context to the clipboard](https://cecli.dev/docs/usage/copypaste.html#copy-aiders-code-context-to-your-clipboard-paste-into-the-web-ui).
...
- Cecli wrote 68% of the code in this release.

### Cecli v0.68.0

- [Cecli works with LLM web chat UIs](https://cecli.dev/docs/usage/copypaste.html).
  - New `--copy-paste` mode.
  - New `/copy-context` command.
- [Set API keys and other environment variables for all providers from command line or YAML conf file](https://cecli.dev/docs/config/cecli_conf.html#storing-llm-keys).
...
- Cecli wrote 71% of the code in this release.

### Cecli v0.67.0

- [Use cecli in your IDE or editor](https://cecli.dev/docs/usage/watch.html).
  - Run `cecli --watch-files` and it will watch for instructions you add to your source files.
  - One-liner `# ...` or `// ...` comments that start or end with "AI" are instructions to cecli.
  - When cecli sees "AI!" it reads and follows all the instructions in AI comments.
...
- [Cecli wrote 61% of the code in this release.](https://cecli.dev/HISTORY.html)
...
- Cecli wrote 82% of the code in this release.
...
- Added `--alias` config to define [custom model aliases](https://cecli.dev/docs/config/model-aliases.html).
...
- Added [RepoMap support for Dart language](https://cecli.dev/docs/languages.html) by @malkoG.
- Ask 2.5% of users if they want to opt-in to [analytics](https://cecli.dev/docs/more/analytics.html).
...
- Cecli wrote 81% of the code in this release.
...
- Added [`/editor` command](https://cecli.dev/docs/usage/commands.html) to open system editor for writing prompts, by @thehunmonkgroup.
...
- Ask 1% of users if they want to opt-in to [analytics](https://cecli.dev/docs/more/analytics.html).
- Added support for [optional multiline input tags](https://cecli.dev/docs/usage/commands.html#entering-multi-line-chat-messages) with matching closing tags.
- Improved [model settings configuration](https://cecli.dev/docs/config/adv-model-settings.html#global-extra-params) with support for global `extra_params` for `litellm.completion()`.
...
- Full support for Claude 3.5 Haiku
  - Scored 75% on [cecli's code editing leaderboard](https://cecli.dev/docs/leaderboards/).
...
- Easily apply file edits from ChatGPT, Claude or other web apps
  ...
  - Run `cecli --apply-clipboard-edits file-to-edit.js`.
  - Cecli will edit your file with the LLM's changes.
- Bugfix for creating new files.
- Cecli wrote 84% of the code in this release.  

### Cecli v0.61.0

- Load and save cecli slash-commands to files:
...
- Anonymous, opt-in [analytics](https://cecli.dev/docs/more/analytics.html) with no personal data sharing.
- Cecli follows litellm's `supports_vision` attribute to enable image support for models.
...
- Full support for Sonnet 10/22, the new SOTA model on cecli's code editing benchmark.
  - Cecli uses Sonnet 10/22 by default.
...
- Cecli wrote 49% of the code in this release.
...
- [Use a pair of Architect/Editor models for improved coding](https://cecli.dev/2024/09/26/architect.html)
...
- Numerous bug fixes.
- Cecli wrote 53% of the code in this release.

### Cecli v0.57.1

- Fixed dependency conflict between bright-vision-core[help] and [playwright].
...
- Cecli wrote 70% of the code in this release.
...
- Numerous bug fixes for corner case crashes.
- Cecli wrote 56% of the code in this release.
...
- When users launch cecli in their home dir, help them find/create a repo in a subdir.
...
- New `--upgrade` switch to install latest version of cecli from pypi.
- Bugfix to `--show-prompt`.
- Disabled automatic reply to the LLM on `/undo` for all models.
- Removed pager from `/web` output.
- Cecli wrote 64% of the code in this release.

### Cecli v0.53.0

- [Keep your prompt cache from expiring](https://cecli.dev/docs/usage/caching.html#preventing-cache-expiration) with `--cache-keepalive-pings`.
...
- New `--install-main-branch` to update to the latest dev version of cecli.
...
- Cecli wrote 59% of the code in this release.
...
- Cecli now offers to run shell commands:
...
- Show the active chat mode at cecli prompt.
...
- Bugfix: properly load `.cecli.models.metadata.json` data.
...
- Cecli wrote 66% of the code in this release.
...
- When [scripting cecli](https://cecli.dev/docs/scripting.html) messages can now contain in-chat `/` commands.
- Cecli in docker image now suggests the correct command to update to latest version.
...
- Enhanced commit message attribution options:
  - New `--attribute-commit-message-author` to prefix commit messages with 'cecli: ' if cecli authored the changes, replaces `--attribute-commit-message`.
  - New `--attribute-commit-message-committer` to prefix all commit messages with 'cecli: '.
- Cecli wrote 61% of the code in this release.
...
- Corrected instructions for upgrading the docker container to latest cecli version.
- Removed obsolete 16k token limit on commit diffs, use per-model limits.

### Cecli v0.48.0

- Performance improvements for large/mono repos.
- Added `--subtree-only` to limit cecli to current directory subtree.
...
- Support for Sonnet's 8k output window.
  - [Cecli already supported infinite output from Sonnet.](https://cecli.dev/2024/07/01/sonnet-not-lazy.html)
- Workaround litellm bug for retrying API server errors.
- Upgraded dependencies, to pick up litellm bug fixes.
- Cecli wrote 44% of the code in this release.
...
- [Commit message](https://cecli.dev/docs/git.html#commit-messages) improvements:
...
- [Linting](https://cecli.dev/docs/usage/lint-test.html) improvements:
...
- Improved [docker support](https://cecli.dev/docs/install/docker.html):
  - Resolved permission issues when using `docker run --user xxx`.
  - New `paulgauthier/cecli-full` docker image, which includes all extras.
- Switching to code and ask mode no longer summarizes the chat history.
- Added graph of cecli's contribution to each release.
...
- Cecli wrote 58% of the code in this release.
...
- New `/chat-mode <mode>` command to switch chat modes:
  ...
  - help: Get help about using cecli (usage, config, troubleshoot).
- Add `file: CONVENTIONS.md` to `.cecli.conf.yml` to always load a specific file.
...
- Stop cecli from asking to add the same url to the chat multiple times.
...
- Cecli wrote 45% of the code in this release.
...
- Cecli is better at offering to add files to the chat on Windows.
...
- Cecli wrote 42% of the code in this release.

### Cecli v0.44.0

- Default pip install size reduced by 3-12x.
- Added 3 package extras, which cecli will offer to install when needed:
  - `bright-vision-core[help]`
  - `bright-vision-core[browser]`
  - `bright-vision-core[playwright]`
...
- Cecli wrote 29% of the code in this release (157/547 lines).
...
- Use `/help <question>` to [ask for help about using cecli](https://cecli.dev/docs/troubleshooting/support.html), customizing settings, troubleshooting, using LLMs, etc.
- Allow multiple use of `/undo`.
- All config/env/yml/json files now load from home, git root, cwd and named command line switch.
- New `$HOME/.cecli/caches` dir for app-wide expendable caches.
- Default `--model-settings-file` is now `.cecli.model.settings.yml`.
- Default `--model-metadata-file` is now `.cecli.model.metadata.json`.
- Bugfix affecting launch with `--no-git`.
- Cecli wrote 9% of the 424 lines edited in this release.
...
- [Allow Claude 3.5 Sonnet to stream back >4k tokens!](https://cecli.dev/2024/07/01/sonnet-not-lazy.html)
...
- Cecli now uses `claude-3-5-sonnet-20240620` by default if `ANTHROPIC_API_KEY` is set in the environment.
- [Enabled image support](https://cecli.dev/docs/usage/images-urls.html) for 3.5 Sonnet and for GPT-4o & 3.5 Sonnet via OpenRouter (by @yamitzky).
- Added `--attribute-commit-message` to prefix cecli's commit messages with "cecli:".
...
- Options to suppress adding "(cecli)" to the [git author and committer names](https://cecli.dev/docs/git.html#commit-attribution).
...
- Use `--sonnet` for Claude 3.5 Sonnet, which is the top model on [cecli's LLM code editing leaderboard](https://cecli.dev/docs/leaderboards/#claude-35-sonnet-takes-the-top-spot).
...
- Commit messages are no longer prefixed with "cecli:". Instead the git author and committer names have "(cecli)" added.

### Cecli v0.38.0

- Use `--vim` for [vim keybindings](https://cecli.dev/docs/usage/commands.html#vi) in the chat.
- [Add LLM metadata](https://cecli.dev/docs/llms/warnings.html#specifying-context-window-size-and-token-costs) via `.cecli.models.json` file (by @caseymcc).
- More detailed [error messages on token limit errors](https://cecli.dev/docs/troubleshooting/token-limits.html).
...
- Cecli will notice if you paste a URL into the chat, and offer to scrape it.
- Performance improvements the repo map, especially in large repos.
- Cecli will not offer to add bare filenames like `make` or `run` which may just be words.
...
- [Cecli can now lint your code and fix any errors](https://cecli.dev/2024/05/22/linting.html).
  - Cecli automatically lints and fixes after every LLM edit.
  - You can manually lint-and-fix files with `/lint` in the chat or `--lint` on the command line.
  - Cecli includes built in basic linters for all supported tree-sitter languages.
  - You can also configure cecli to use your preferred linter with `--lint-cmd`.
- Cecli has additional support for running tests and fixing problems.
  - Configure your testing command with `--test-cmd`.
  - Run tests with `/test` or from the command line with `--test`.
  - Cecli will automatically attempt to fix any test failures.
  

### Cecli v0.35.0

- Cecli now uses GPT-4o by default.
  - GPT-4o tops the [cecli LLM code editing leaderboard](https://cecli.dev/docs/leaderboards/) at 72.9%, versus 68.4% for Opus.
  - GPT-4o takes second on [cecli's refactoring leaderboard](https://cecli.dev/docs/leaderboards/#code-refactoring-leaderboard) with 62.9%, versus Opus at 72.3%.
...
- Added native support for [Deepseek models](https://cecli.dev/docs/llms.html#deepseek) using `DEEPSEEK_API_KEY` and `deepseek/deepseek-chat`, etc rather than as a generic OpenAI compatible API.

### Cecli v0.32.0

- [Cecli LLM code editing leaderboards](https://cecli.dev/docs/leaderboards/) that rank popular models according to their ability to edit code.
...
- [Cecli is now also AI pair programming in your browser!](https://cecli.dev/2024/05/02/browser.html) Use the `--browser` switch to launch an experimental browser based version of cecli.
...
- Added [Gemini 1.5 Pro](https://cecli.dev/docs/llms.html#free-models) as a recommended free model.
...
- Improved [model warnings](https://cecli.dev/docs/llms.html#model-warnings) for unknown or unfamiliar models
...
- Added support for [directly connecting to Anthropic, Cohere, Gemini and many other LLM providers](https://cecli.dev/docs/llms.html).
...
- New command line switches for working with popular models:
  ...
  - `--sonnet`
  - `--anthropic-api-key`
- Improved "whole" and "diff" backends to better support [Cohere's free to use Command-R+ model](https://cecli.dev/docs/llms.html#cohere).
...
- Added support for new `gpt-4-turbo-2024-04-09` and `gpt-4-turbo` models.
  - Benchmarked at 61.7% on Exercism benchmark, comparable to `gpt-4-0613` and worse than the `gpt-4-preview-XXXX` models. See [recent Exercism benchmark results](https://cecli.dev/2024/03/08/claude-3.html).
  - Benchmarked at 34.1% on the refactoring/laziness benchmark, significantly worse than the `gpt-4-preview-XXXX` models. See [recent refactor bencmark results](https://cecli.dev/2024/01/25/benchmarks-0125.html).
  - Cecli continues to default to `gpt-4-1106-preview` as it performs best on both benchmarks, and significantly better on the refactoring/laziness benchmark.
...
- Issue a warning if user adds too much code to the chat.
  - https://cecli.dev/docs/faq.html#how-can-i-add-all-the-files-to-the-chat
...
- Made past articles part of https://cecli.dev/blog/
...
- [Significantly reduced "lazy" coding from GPT-4 Turbo due to new unified diff edit format](https://cecli.dev/docs/unified-diffs.html)
  - Score improves from 20% to 61% on new "laziness benchmark".
  - Cecli now uses unified diffs by default for `gpt-4-1106-preview`.
...
- [Benchmarks for OpenAI's new 11/06 models](https://cecli.dev/docs/benchmarks-1106.html)
- Streamlined [API for scripting cecli, added docs](https://cecli.dev/docs/faq.html#can-i-script-cecli)
- Ask for more concise SEARCH/REPLACE blocks. [Benchmarked](https://cecli.dev/docs/benchmarks.html) at 63.9%, no regression.
...
- [Improved repository map using tree-sitter](https://cecli.dev/docs/repomap.html)
- Switched from "edit block" to "search/replace block", which reduced malformed edit blocks. [Benchmarked](https://cecli.dev/docs/benchmarks.html) at 66.2%, no regression.
- Improved handling of malformed edit blocks targeting multiple edits to the same file. [Benchmarked](https://cecli.dev/docs/benchmarks.html) at 65.4%, no regression.
- Bugfix to properly handle malformed `/add` wildcards.


### Cecli v0.15.0

- Added support for `.aiderignore` file, which instructs cecli to ignore parts of the git repo.
- New `--commit` cmd line arg, which just commits all pending changes with a sensible commit message generated by gpt-3.5.
- Added universal ctags and multiple architectures to the [cecli docker image](https://cecli.dev/docs/install/docker.html)
- `/run` and `/git` now accept full shell commands, like: `/run (cd subdir; ls)`
- Restored missing `--encoding` cmd line switch.

### Cecli v0.14.2

- Easily [run cecli from a docker image](https://cecli.dev/docs/install/docker.html)
...
- Fixed /add bug in when cecli launched in repo subdir
- Show models supported by api/key if requested model isn't available

### Cecli v0.14.0

- [Support for Claude2 and other LLMs via OpenRouter](https://cecli.dev/docs/faq.html#accessing-other-llms-with-openrouter) by @joshuavial
- Documentation for [running the cecli benchmarking suite](https://github.com/dwash96/cecli/tree/main/benchmark)
- Cecli now requires Python >= 3.9


### Cecli v0.13.0

- [Only git commit dirty files that GPT tries to edit](https://cecli.dev/docs/faq.html#how-did-v0130-change-git-usage)
...
- Late-bind importing `sounddevice`, as it was slowing down cecli startup
- Improved --foo/--no-foo switch handling for command line and yml config settings

### Cecli v0.12.0

- [Voice-to-code](https://cecli.dev/docs/usage/voice.html) support, which allows you to code with your voice.
- Fixed bug where /diff was causing crash.
- Improved prompting for gpt-4, refactor of editblock coder.
- [Benchmarked](https://cecli.dev/docs/benchmarks.html) at 63.2% for gpt-4/diff, no regression.
...
- [Benchmarked](https://cecli.dev/docs/benchmarks.html) at 53.4% for gpt-3.5/whole (no regression).
...
- Stronger GPT-3.5 prompt against skipping/eliding code in replies (51.9% [benchmark](https://cecli.dev/docs/benchmarks.html), no regression)
...
- Added `/git` command to run git from inside cecli chats.
- Use Meta-ENTER (Esc+ENTER in some environments) to enter multiline chat messages.
- Create a `.gitignore` with `.cecli*` to prevent users from accidentally adding cecli files to git.
- Check pypi for newer versions and notify user.
- Updated keyboard interrupt logic so that 2 ^C in 2 seconds always forces cecli to exit.
- Provide GPT with detailed error if it makes a bad edit block, ask for a retry.
- Force `--no-pretty` if cecli detects it is running inside a VSCode terminal.
- [Benchmarked](https://cecli.dev/docs/benchmarks.html) at 64.7% for gpt-4/diff (no regression)


### Cecli v0.9.0

- Support for the OpenAI models in [Azure](https://cecli.dev/docs/faq.html#azure)
- Added `--show-repo-map`
...
- [Benchmarked](https://cecli.dev/docs/benchmarks.html) at 53.8% for gpt-3.5-turbo/whole (no regression)

### Cecli v0.8.3

- Added `--dark-mode` and `--light-mode` to select colors optimized for terminal background
- Install docs link to [NeoVim plugin](https://github.com/joshuavial/cecli.nvim) by @joshuavial
...
- Bugfix so that cecli throws an exception when OpenAI returns InvalidRequest
...
- Added [FAQ](https://cecli.dev/docs/faq.html)
...
- [Benchmark comparing code editing in GPT-3.5 and GPT-4](https://cecli.dev/docs/benchmarks.html)
...
- Fixed a bug to allow cecli to edit files that contain triple backtick fences.
